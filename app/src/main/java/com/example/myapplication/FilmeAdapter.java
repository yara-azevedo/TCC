...
public class FilmeAdapter extends FirestoreRecyclerAdapter<Conteudo, FilmeAdapter.ConteudoModelViewHolder> {
    Context context;


    public FilmeAdapter(@NonNull FirestoreRecyclerOptions<Conteudo> options, Context context) {
        super(options);
        this.context = context;
    }

    @Override
    protected void onBindViewHolder(@NonNull FilmeAdapter.ConteudoModelViewHolder holder, int position, @NonNull Conteudo conteudo) {
    ...
    @NonNull
    @Override
    public FilmeAdapter.ConteudoModelViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.conteudo_item,parent, false);
        return new FilmeAdapter.ConteudoModelViewHolder(view);
    }
