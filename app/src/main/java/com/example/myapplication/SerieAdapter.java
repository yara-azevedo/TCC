...
public class SerieAdapter extends FirestoreRecyclerAdapter<Conteudo, SerieAdapter.ConteudoModelViewHolder> {
    Context context;


    public SerieAdapter(@NonNull FirestoreRecyclerOptions<Conteudo> options, Context context) {
        super(options);
    ...
    @NonNull
    @Override
    public SerieAdapter.ConteudoModelViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.conteudo_item,parent, false);
        return new SerieAdapter.ConteudoModelViewHolder(view);
    }
