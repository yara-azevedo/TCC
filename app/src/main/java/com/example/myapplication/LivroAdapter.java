...
public class LivroAdapter extends FirestoreRecyclerAdapter<Conteudo, LivroAdapter.ConteudoModelViewHolder> {
    Context context;


    public LivroAdapter(@NonNull FirestoreRecyclerOptions<Conteudo> options, Context context) {
        super(options);
    ...
    @NonNull
    @Override
    public LivroAdapter.ConteudoModelViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.conteudo_item,parent, false);
        return new LivroAdapter.ConteudoModelViewHolder(view);
    }
